...
async function resolveYouTubeTarget(m, args, usedPrefix, command) {
          ...
          if (matchingItem) {
            if (index < matchingItem.urls.length) youtubeLink = matchingItem.urls[index]
              ...
              await conn.sendMessage(m.chat, { 
                text: `${lenguajeGB['smsAvisoFG']()} ${mid.smsYT} ${matchingItem.urls.length}*`
              }, { quoted: m });
              return null;
            }
          } else {
            await conn.sendMessage(m.chat, { 
              text: `${lenguajeGB['smsAvisoMG']()} ${mid.smsY2(usedPrefix, command)} ${usedPrefix}playlist <texto>*`
            }, { quoted: m });
            return null;
          }
        } else {
  ...
  if (!target) {
    await conn.sendMessage(m.chat, { 
      text: '❌ No se encontró URL válida de YouTube.'
    }, { quoted: m });
    return null;
  }
...
const handler = async (m, { conn, command, args, text, usedPrefix }) => {
  ...
  try {
    ...
    await conn.sendMessage(m.chat, { 
      text: `${lenguajeGB['smsAvisoEG']()}𝙋𝙍𝙊𝙉𝙏𝙊 𝙏𝙀𝙉𝘿𝙍𝘼 𝙎𝙐 𝘿𝙊𝘾𝙐𝙈𝙀𝙉𝙏𝙊 ${etiqueta}, 𝙀𝙎𝙋𝙀𝙍𝙀 𝙋𝙊𝙍 𝙁𝘼𝙑𝙊𝙍\n\n𝙎𝙊𝙊𝙉 𝙔𝙊𝙐 𝙒𝙄𝙇𝙇 𝙃𝘼𝙑𝙀 𝙔𝙊𝙐𝙍 ${etiqueta} 𝘿𝙊𝘾𝙐𝙈𝙀𝙉𝙏, 𝙋𝙇𝙀𝘼𝙎𝙀 𝙒𝘼𝙄𝙏`
    }, { quoted: m });
