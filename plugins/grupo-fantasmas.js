...
let handler = async (m, { conn, text, participants, args, command }) => {
    ...
    switch (command) {
        case "fantasmas":
            if (total == 0) return await conn.sendMessage(
                m.chat,
                { text: `${lenguajeGB['smsAvisoAG']()}𝙀𝙎𝙏𝙀 𝙂𝙍𝙐𝙋𝙊 𝙀𝙎 𝘼𝘾𝙏𝙄𝙑𝙊 𝙉𝙊 𝙏𝙄𝙀𝙉𝙀 𝙁𝘼𝙉𝙏𝘼𝙎𝙈𝘼𝙎 :D` },
                { quoted: m }
            );
