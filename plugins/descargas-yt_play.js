...
if (!text) return await conn.sendMessage(m.chat, { text: `${lenguajeGB['smsAvisoMG']()}${mid.smsMalused4}\n*${usedPrefix + command} Billie Eilish - Bellyache*` }, { quoted: m });
...
try {
if (text === '🎶' || text === 'audio') {
await conn.sendMessage(m.chat, { text: lenguajeGB['smsAvisoEG']() + mid.smsAud }, { quoted: m || null });
...
await conn.sendMessage(m.chat, { text: '❌ No se pudo descargar el audio' }, { quoted: m || null })
}
} else if (text === '📽' || text === 'video') {
await conn.sendMessage(m.chat, { text: lenguajeGB['smsAvisoEG']() + mid.smsVid }, { quoted: m || null });
...
await conn.sendMessage(m.chat, { text: '❌ No se pudo descargar el video' }, { quoted: m || null });
}}} catch (error) {
console.error(error);
} finally {
